...
<body>
      ...
      <div class="max-w-screen-xl px-4 py-8 mx-auto sm:px-6 sm:py-12 lg:px-4	">
	    ...
	    <c:if test="${pgList.count==0}">
			<ul class="grid grid-cols-1 gap-8 mt-8 lg:grid-cols-1">
	           	<li>
		            <a href="#" class="relative block overflow-hidden group rounded-lg"> 
		              <img
		                src="img/img_event/이벤트-준비중2.png"
		                alt=""
	                  ...
	                  <div class="relative p-6 bg-white border border-gray-100">
	                  	<h3 class="mt-4 text-lg font-medium text-gray-900">현재 진행중인 이벤트가 없습니다.</h3>
	                  </div>
	           		</a>
	         		</li>
	       	</ul>
          ...
          <li>
            <a href="./E_content.shop?num=${article.eventno}&pageNum=${pgList.currentPage}" class="relative block overflow-hidden group rounded-lg"> 
              ...
              <div class="relative p-6 bg-white border border-gray-100">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900">${article.subject}</h3>
            	
                <p class="mt-1.5 text-sm text-gray-700"><span>${article.eventstart}</span> - <span>${article.eventend}</span></p>
                <p class="mt-1.5 text-sm text-gray-700 text-right">
                	<span>
                		글번호 : <c:out value="${number}"/>
    					<c:set var="number" value="${number-1}"/>
    					<br>
    					조회수 : ${article.readcount}
                	</span>
                </p>
                <p class="mt-1.5 text-sm text-gray-700 text-right">작성일 : <fmt:formatDate value="${article.eventdate}" timeStyle="medium" pattern="yy.MM.dd (hh:mm)"/>
                ...
                <!-- <span
                  class="whitespace-nowrap bg-gray-400 px-3 py-1.5 text-xs font-medium"
                >
                  종료
                </span> -->
        </ul>
	...
	<form name="test" action="./E_event.shop">
        ...
        <div class="mx-auto max-w-screen-2xl px-4 py-3 sm:px-6 lg:px-8">
          <div class="flex flex splace-x-4 items-center  sm:gap-4">
			<div class="float-right">
			    <select name="search" class="h-12 w-full rounded-lg border border-gray-200 bg-white  pl-4 pr-10 text-sm sm:w-[150px] ">
            ...
            <div class="relative hidden sm:block">
              ...
              <button type="submit" class="absolute top-1/2 right-2 -translate-y-1/2 rounded-md bg-gray-50 p-2 text-gray-600 transition hover:text-gray-700">
